...
class LiquidEdge {
  List<LiquidPoint> points = [];
  Side side;
  double edgeTension = 0.1;
  ...
  LiquidEdge({count = 10, this.side = Side.left}) {
    for (int i = 0; i < count; i++) {
      points.add(LiquidPoint(0.0, i / (count - 1)));
    }
  }
  ...
  void applyTouchOffset([Size? size, Offset? offset]) {
  ...
  Path buildPath(Size size, {double margin = 0.0}) {
    ...
    Path path = Path();
    int l = points.length;
    Offset pt = LiquidPoint(-margin, 1.0).toOffset(mtx), pt1;
    path.moveTo(pt.dx, pt.dy); // bl

    pt = LiquidPoint(-margin, 0.0).toOffset(mtx);
  ...
  void tick(Duration duration) {
    ...
    double dampingT = pow(damping, t).toDouble();

    for (int i = 0; i < l; i++) {
      LiquidPoint pt = points[i];
  ...
  void _addPointTension(LiquidPoint pt0, double x, double t) {
...
class LiquidPoint {
  ...
  LiquidPoint([this.x = 0.0, this.y = 0.0]);
